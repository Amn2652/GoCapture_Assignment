...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class DepthOfField : PostEffectsBase {
        ...
        private void WriteCoc(RenderTexture fromTo, bool fgDilate)
        {
            ...
            {
                // The rest of your nearBlur logic
            }
            else
            {
                // Create a temporary RenderTexture
                RenderTexture temp = RenderTexture.GetTemporary(fromTo.width, fromTo.height, 0, fromTo.format);

                // Blit to the temporary texture first
                Graphics.Blit(fromTo, temp, dofHdrMaterial, 0);

                // Blit back to the original texture
                Graphics.Blit(temp, fromTo);
